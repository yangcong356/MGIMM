...
QUESTION = [
    '<image>\n Based on the provided region of the remote sensing image, describe the basic attributes of the main objects in that region.'
    ]

class DIORVGDataset(Dataset):
    """Dataset for supervised fine-tuning."""

    def __init__(self, annotations_path: str,
                 tokenizer: transformers.PreTrainedTokenizer,
                 data_args):
        super(DIORVGDataset, self).__init__()
        ...
        for i in data_dict['images']:
            self.image_index.append({i['id']:i['file_name'],
                                     str(i['id'])+'_size':(i['width'], i['height'])})
    ...
    def process_vgdata2llava(self, ann_infos):
        sources = {'conversations': []}
        question = random.choice(QUESTION).strip()
        sources['conversations'].append({'from': 'human', 'value': question})
        sources['conversations'].append({'from': 'gpt', 'value': ann_infos['caption']})
        return sources
    ...
    def __getitem__(self, index) -> Dict[str, torch.Tensor]:
        ...
        # pdb.set_trace()
        if 'image_id' in ann:
            image_file_name = next((record[ann["image_id"]] for record in self.image_index if ann["image_id"] in record), None)
            image_folder = self.data_args.image_folder
            processor = self.data_args.image_processor
            image = Image.open(os.path.join(image_folder, image_file_name)).convert('RGB')
            image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
            sources = self.process_vgdata2llava(ann)
        ...
        if 'bbox' in ann:
            ...
            input_img_size = next((record[str(ann['image_id'])+'_size'] for record in self.image_index if str(ann['image_id'])+'_size' in record), None)
            target_length = self.data_args.input_image_size
            bbox = apply_boxes(bbox, input_img_size, target_length)
            bbox_torch = torch.as_tensor(bbox, dtype=torch.bfloat16)
...
def make_supervised_data_module(tokenizer,
    ...
    train_dataset = build_dataset(dataset_config['datasets'],
                                tokenizer=tokenizer,
                                data_args=data_args)
...
def build_dataset(dataset_config,
    ...
    dataset_annotations = dataset_config.pop('annotations')
    # ratio = dataset_config.pop('ratio', 1)
    if dataset_name == 'diorvg':
        dataset = DIORVGDataset(
            dataset_annotations,
            tokenizer=tokenizer,
            data_args=data_args
        )
